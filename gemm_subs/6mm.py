...
from cutlass.cute.nvgpu.tcgen05 import MmaMXF4NVF4Op, OperandSource

# Kernel configuration parameters
# Tile sizes for M, N, K dimensions
# mma_tiler_mnk = (128, 128, 256)
mma_tiler_mn_T = (256, 64)
cluster_mn_T = (2, 1) 
mma_tiler_mn_nT = (128, 64)
cluster_mn_nT = (1, 1)
...
// Kernel Perf config
using MmaTileShape        = Shape<_128,_64,_256>;                          // MMA's tile size
...
void gemm_cuda(
    ...
    typename Gemm::Arguments arguments {
        ...
        {128, 7168, 16384, 1},
        {
        reinterpret_cast<ElementA::DataType const*>(a.data_ptr()), stride_A,
        reinterpret_cast<ElementB::DataType const*>(b.data_ptr()), stride_B,
        reinterpret_cast<ElementA::ScaleFactorType const*>(sfa.data_ptr()), layout_SFA,
        reinterpret_cast<ElementB::ScaleFactorType const*>(sfb.data_ptr()), layout_SFB
        },
        {
        {1.0f, 0.0f},
    ...
    gemm.initialize(arguments, nullptr);
    gemm.run();
}
"""
...
class Sm100BlockScaledPersistentDenseGemmKernel:
    ...
    def __init__(
        ...
        mma_tiler_mn = mma_tiler_mn_T if transposed else mma_tiler_mn_nT
        cluster_shape_mn = cluster_mn_T if transposed else cluster_mn_nT
    ...
    def __call__(
        ...
        if cutlass.const_expr(self.transposed):
            a_tensor = cute.make_tensor(
                b_ptr,
                cute.make_layout(
                    (n, k, l),
            ...
            c_tensor = cute.make_tensor(
                c_ptr, cute.make_layout((n, m, l), stride=(1, n, m * n))
            )
            sfa_ptr, sfb_ptr = sfb_ptr, sfa_ptr
        ...
        # cute.printf(grid)
        # cute.printf(self.cluster_shape_mn)
        return
    ...
    def kernel(
        ...
        # while work_tile.is_valid_tile:
            ...
            #     cur_tile_coord[2],
            # )
            mma_tile_coord_mnl = (bidz % 28, bidz // 28, 0) if cutlass.const_expr(self.transposed) else (0,bidz,0)
        ...
        # while work_tile.is_valid_tile:
            ...
            # if tidx == 0 and bidz == 40 and bidx == 0:
            #     cute.printf(mma_tile_coord_mnl)
            #     cute.printf(bidz)
            #     cute.printf(bidx)
                # cute.printf(mma_tile_coord_mnl)
            mma_tile_coord_mnl = (bidz % 28, bidz // 28, 0) if cutlass.const_expr(self.transposed) else (0,bidz,0)
            # if cutlass.const_expr(self.transposed):
            #     mma_tile_coord_mnl = (bidz % 28, bidz // 28, 0)
            #
            # Slice to per mma tile index
            #
...
def compile_kernel(problem_size):
    ...
    _,n,k,_ = problem_size
    transposed = (k == 16384 and n == 7168)
    # transposed = False

    # Compile the kernel
    _compiled_kernel_cache[problem_size] = cute.compile(Sm100BlockScaledPersistentDenseGemmKernel(sf_vec_size, transposed), a_ptr, b_ptr, sfa_ptr, sfb_ptr, c_ptr, problem_size)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m==128 and ((n, k) in [(7168, 16384), (4096, 7168), (7168,2048)]):
    ...
    # clear_l2_cache()
    # ref_kernel(data)
    # if k == 16384 and n == 7168:
    #     module.gemm_cuda(a,b,sfa_permuted,sfb_permuted,c)
    # else:
    #     ref_kernel(data)
    return c
