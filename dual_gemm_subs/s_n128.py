...
void dual_gemm_silu_kernel(
    ...
    // TMEM layout for 128x128:
    // GEMM1 accumulator: columns [0, 128)
    // GEMM2 accumulator: columns [128, 256)
    // Scale factors: columns [256, ...)
    constexpr int GEMM1_D_TMEM = 0;
    constexpr int GEMM2_D_TMEM = BLOCK_N;  // 128
    constexpr int SFA_tmem = BLOCK_N * 2;  // 256
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        // Allocate TMEM: 2 * BLOCK_N columns for accumulators + scale factors
    ...
    if (warp_id == NUM_WARPS - 2 && elect_sync()) {
        ...
        // MMA warp: run both GEMMs with separate accumulators
        ...
        for (int iter_k = 0; iter_k < num_iters; iter_k++) {
            ...
            for (int k1 = 0; k1 < BLOCK_K / 256; k1++)
                for (int k2 = 0; k2 < 256 / MMA_K; k2++) {
                    ...
                    int k_sf = k1 * 4 + k2;
                    // BLOCK_M=128 and BLOCK_N=128, so offsets are 0
                    const int scale_A_tmem = SFA_tmem + k_sf * 4;
                    const int scale_B_tmem = SFB_tmem + k_sf * 4;
...
dual_gemm_silu = torch.ops.dual_gemm_n128.dual_gemm_silu
